...
- Python 3.10+
- 8GB+ VRAM (for Mistral-7B 4-bit) or any GPU (for TinyLlama)
- 16GB RAM recommended

### 1. Clone & Install

```bash
git clone https://github.com/vikramaditya005/rag-system.git
cd rag-system
...
# LLM_MODEL_NAME=TinyLlama/TinyLlama-1.1B-Chat-v1.0   ← fast, any GPU
# LLM_MODEL_NAME=mistralai/Mistral-7B-Instruct-v0.2   ← powerful, needs 8GB VRAM
# USE_4BIT=true                                         ← enable for Mistral on 8GB GPU
```

### 3. Build the Index (one-time, ~10 min)

```bash
python -m scripts.build_index --query "large language models RAG" --max 50
```

Downloads 50 arXiv papers, chunks them, embeds them, and saves a FAISS index.
...
```bash
# Fast test (20 samples, ~30 min)
python experiments/run_eval.py --skip-ingest --max-samples 20

# Full evaluation (50 samples, ~2 hours)
python experiments/run_eval.py --skip-ingest --max-samples 50
```

Results saved to `experiments/results/metrics_summary.json`
...
{
  "question": "How does RAG reduce hallucinations?",
  "answer": "RAG reduces hallucinations by grounding the model...",
  "mode": "rag",
  "sources": [
    { "paper_title": "Retrieval-Augmented Generation for...", "score": 1.8024, "rank": 1 }
  ],
  "latency": { "embed_ms": 324, "search_ms": 27, "rerank_ms": 1343, "total_ms": 17097 },
  "token_usage": { "prompt_tokens": 512, "completion_tokens": 187 }
}
```

### `POST /compare`
Runs the same question in both RAG and baseline mode and returns both answers side by side.
...
### `GET /index-stats`
```json
{ "total_vectors": 1059, "total_chunks": 1059 }
```
